...
public class BluetoothLeClient {
    ...
    private  BluetoothGattCallback mGattCallback = new BluetoothGattCallback() {
		...
		public void onConnectionStateChange(BluetoothGatt gatt, int status,
            ...
            if (newState == BluetoothProfile.STATE_CONNECTED) {
                ...
                if(mCallback != null) mCallback.ConnectStatus(true, mBluetoothDeviceAddress);
                ...
                if(mCallback != null) mCallback.ConnectStatus(false, mBluetoothDeviceAddress);
            }
		}
		...
		public void onCharacteristicRead(BluetoothGatt gatt,
			...
			BleLog.i(TAG, "onCharacteristicRead() : characteristic = " + characteristic.getUuid().toString() + " status = " + status);
			 ...
			 if(mDevice != null && !disconnectRequired) {
				 ...
				 if(mCallback != null)  mCallback.DataCallback(data, mBluetoothDeviceAddress);
			 }
    ...
    private boolean RegisterDevice(BluetoothDevice device) {
    	...
    	if(mBluetoothDeviceName.contains("UMHD")) {
    		mDevice = new UMHDDevice(device);
        	BleLog.d(TAG, "RegisterDevice() : UMHDDevice");
        	return true;
    	}
    	
    	if(mBluetoothDeviceName.contains("Bracelet")) {
    		mDevice = new IwownI5Device(device);
        	BleLog.d(TAG, "RegisterDevice() : IwownI5Device");
        	return true;
    	} 
   	
    	if(mBluetoothDeviceName.contains("M04_L")) {
    		mDevice = new M04LDevice(device);
        	BleLog.d(TAG, "RegisterDevice() : M04LDevice");
        	return true;
    	} 
